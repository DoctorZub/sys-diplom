...
Повторюсь, что настройка всех ВМ осуществляется удаленно с рабочей станции администратора через Ansible playbooks, а подключение ssh осуществляется через *bastion* host (или, другое название, Jump Host). Для реализации данной концепции на рабочей машине администратора необходимо внести изменения в файл `~/.ssh/config`:
```
...
2. Создается 3 подсети:
   ...
   - *develop_a_pub* в зоне `ru-central1-a` CIDR `192.168.0.0/24`  
     К данной подсети подключены ВМ, имеющие внешний IP-адрес и доступ к общению через интернет: *zabbix, kibana, ALB*.
3. Создаются группы безопасности (настройки firewall'a):
   ...
   `Входящий трафик`:
    ...
    `Исходящий трафик`:
      - без ограничений.  
**<ins>Далее группы безопасности подключаются только к одноименным серверам</ins>**
...
- Информация о том, сколько времени процессор тратит на выполнение различных процедур - ожидание ввода/вывода, системные прерывания, время бездействия и т.д.
...
Конфигурация [*elasticsearch*](https://github.com/DoctorZub/sys-diplom/blob/main/main/ansible/configs/elasticsearch/elasticsearch.yml):
...
Конфигурация [*kibana*](https://github.com/DoctorZub/sys-diplom/blob/main/main/ansible/configs/kibana/kibana.yml):
...
Для отображения данных с Elasticsearch в Kibana было совершено несколько подключений к серверам *web-a и web-b* через публичный адрес ALB.  
...
- `log.file.path` - лог-файл, с которого считано сообщение;
- `referrer` - HTTP-Referrer;
- `url` - на какой путь выполнялось подключение к серверу;
- `remote_agent` - с какого агента/приложения выполнялось подключение.

Можно заметить, что с самого начала логи засорены подключением от агента *Envoy/HC*

![Kibana-4](https://github.com/DoctorZub/sys-diplom/blob/main/main/img/kibana_4.png) 

Этим подключением является *healthcheck*, который проводит балансировщик нагрузки. Для удобства отслеживания других подключений и ошибок, можно убрать из анализа сообщения от данного агента.

![Kibana-4](https://github.com/DoctorZub/sys-diplom/blob/main/main/img/kibana_5.png)
...
![Snapshot](https://github.com/DoctorZub/sys-diplom/blob/main/main/img/snap.png)

---
### Заключение
В данной работе успешно удалось: 
1. Разработать и развернуть отказоустойчивую инфраструктуру для сайта с помощью средств `Terraform`, `Ansible` и `Yandex Cloud`;
2. Настроить мониторинг серверов с помощью `Zabbix Server` и `Zabbix Agents2`, создать *Dashboards* для удобного визуального представления собираемых метрик;
3. Настроить `ELK-стек` на сбор, фильтрацию и удобное визуальное представление логов с серверов;
4. Обеспечить безопасное сетевое взаимодействие для всех сервисов внутри и за пределами облачной сети.

Направлением для развития и улучшения данной инфраструктуры является:
- Более глубокая настройка функций безопасности (аутентификация и автозирация в Elasticsearch, использование TLS и HTTPS);
- Репликация кластера Elasticsearch;
- Увеличение отслеживаемых метрик в Zabbix.
